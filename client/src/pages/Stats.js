    ...
    function predictor(expensesVal, monthlyEarnings, predictionMonth) {
        ...
        if (predictionMonth < xUnitVal.length + 1) { alert("please make sure you are asking to predict in the future!, make sure to add one month") }
            ...
            model.fit(xs, ys, { epochs: 100 }).then(() => {
                ...
                // console.log(model.predict(tf.tensor2d([parseFloat(predictionMonth)], [1, 1])).toString()[14])
                // console.log(model.predict(tf.tensor2d([parseFloat(predictionMonth)], [1, 1])).toString()[15])
                // console.log(model.predict(tf.tensor2d([parseFloat(predictionMonth)], [1, 1])).toString()[16])
                // console.log(model.predict(tf.tensor2d([parseFloat(predictionMonth)], [1, 1])).toString()[17])
                // console.log(model.predict(tf.tensor2d([parseFloat(predictionMonth)], [1, 1])).toString()[18])
                // console.log(model.predict(tf.tensor2d([parseFloat(predictionMonth)], [1, 1])).toString()[19])
                // console.log(model.predict(tf.tensor2d([parseFloat(predictionMonth)], [1, 1])).toString()[20])
                // console.log(model.predict(tf.tensor2d([parseFloat(predictionMonth)], [1, 1])).toString()[21])
                // console.log(model.predict(tf.tensor2d([parseFloat(predictionMonth)], [1, 1])).toString()[22])
                // console.log(model.predict(tf.tensor2d([parseFloat(predictionMonth)], [1, 1])).toString()[23])
